export function registerFlightTools(engine) {
  ...
  const read = () => ({ running: engine.running, physicsEngine:'JSBSim',physicsStatus:engine.physicsStatus,acrobatic:!!engine.state.acrobatic, scenery: engine.mode, altitudeFeet: Math.round(engine.state.y * 3.28084), airspeedKnots: Math.round(engine.state.speed * 1.94384) });
  const tools = [
    ...
    { name: 'set_flight_paused', description: 'Pause or resume the current flight. Does not reset a crashed flight.', inputSchema: { type: 'object', properties: { paused: { type: 'boolean' } }, required: ['paused'], additionalProperties: false }, annotations: { readOnlyHint: false }, execute: input => {
      ...
      if (!input.paused && (!engine.google || !engine.flight?.initialized)) throw new Error('Choose and connect scenery before starting flight.');
